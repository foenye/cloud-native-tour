...
func (r *mockedRouter) ResolveEndpoint(_, _ string, _ int32) (*url.URL, error) {
	...
	tests := map[string]struct {
		...
		"no user": {
			apiService: &apiregistrationv1.APIService{
				...
				Spec: apiregistrationv1.APIServiceSpec{
					Service: &apiregistrationv1.ServiceReference{Port: ptr.To[int32](443)},
					Group:   "foo",
					Version: "v1",
				},
		...
		"proxy with user, cabundle": {
			...
			apiService: &apiregistrationv1.APIService{
				ObjectMeta: metav1.ObjectMeta{Name: "v1.foo"},
				Spec: apiregistrationv1.APIServiceSpec{
					Service:  &apiregistrationv1.ServiceReference{Name: "test-service", Namespace: "test-ns", Port: ptr.To[int32](443)},
					Group:    "foo",
					Version:  "v1",
					CABundle: testCACrt,
				},
		...
		"service unresolveable": {
			...
			apiService: &apiregistrationv1.APIService{
				ObjectMeta: metav1.ObjectMeta{Name: "v1.foo"},
				Spec: apiregistrationv1.APIServiceSpec{
					Service:  &apiregistrationv1.ServiceReference{Name: "bad-service", Namespace: "test-ns", Port: ptr.To[int32](443)},
					Group:    "foo",
					Version:  "v1",
					CABundle: testCACrt,
				},
...
func (m *mockEgressDialer) dialBroken(_ context.Context, _, _ string) (net.Conn, error) {
	m.called++
		...
		"invalid hostname + insecure": {
			APIService: &apiregistrationv1.APIService{
				Spec: apiregistrationv1.APIServiceSpec{
					...
					Group:                 "mygroup",
					Version:               "v1",
					Service:               &apiregistrationv1.ServiceReference{Name: "invalid-service", Namespace: "invalid-ns", Port: ptr.To[int32](443)},
				},
				Status: apiregistrationv1.APIServiceStatus{
					Conditions: []apiregistrationv1.APIServiceCondition{
						{Type: apiregistrationv1.Available, Status: apiregistrationv1.ConditionTrue},
	...
	for k, tc := range testcases {
		...
		t.Run(tcName, func(t *testing.T) {
			...
			backendHandler.Handle(path, websocket.Handler(func(ws *websocket.Conn) {
				...
				req := ws.Request()
				userinfo := req.Header.Get("X-Remote-User")
				if userinfo != upgradeUser {
					t.Errorf("expected user %q, got %q", upgradeUser, userinfo)
				}
				...
				body := make([]byte, 5)
				_, _ = ws.Read(body)
				_, _ = ws.Write([]byte("hello " + string(body)))
			}))
...
func TestGetContextForNewRequest(t *testing.T) {
	...
	proxyServer := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, req *http.Request) {
		...
		theproxy := proxy.NewUpgradeAwareHandler(location, server.Client().Transport, true, false, &responder{response: w})
...
func TestTracerProvider(t *testing.T) {
	...
	server := httptest.NewServer(contextHandler(filters.WithTracing(handler, otelTracer), userinfo))
	defer server.Close()
...
func TestProxyCertReload(t *testing.T) {
	...
	apiService := &apiregistrationv1.APIService{
		ObjectMeta: metav1.ObjectMeta{Name: "v1.foo"},
		Spec: apiregistrationv1.APIServiceSpec{
			Service:  &apiregistrationv1.ServiceReference{Name: "test-service2", Namespace: "test-ns", Port: ptr.To[int32](443)},
			Group:    "foo",
			Version:  "v1",
