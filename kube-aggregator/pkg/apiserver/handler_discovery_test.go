...
func newDiscoveryManager(rm endpointsDiscoveryAggregated.ResourceManager) *discoveryManager {
...
func TestBasic(t *testing.T) {
	service1 := endpointsDiscoveryAggregated.NewResourceManager("apis")
	service2 := endpointsDiscoveryAggregated.NewResourceManager("apis")
	service3 := endpointsDiscoveryAggregated.NewResourceManager("apis")
	...
	aggregatedResourceManager := endpointsDiscoveryAggregated.NewResourceManager("apis")
...
func TestInitialRunHasAllAPIServices(t *testing.T) {
	...
	service := endpointsDiscoveryAggregated.NewResourceManager("apis")
	aggregatedResourceManager := endpointsDiscoveryAggregated.NewResourceManager("apis")
...
func TestRemoveAPIService(t *testing.T) {
	aggyService := endpointsDiscoveryAggregated.NewResourceManager("apis")
	service := endpointsDiscoveryAggregated.NewResourceManager("apis")
...
func fetchPath(handler http.Handler, etag string) (*http.Response, []byte, *apiDiscoveryV2.APIGroupDiscoveryList) {
	...
	if len(bytes) > 0 {
		...
		_ = runtime.DecodeInto(scheme.Codecs.UniversalDecoder(), bytes, decoded)
	}
...
func (q *completerWorkqueue) AddAfter(item string, _ time.Duration) {
	q.Add(item)
}
